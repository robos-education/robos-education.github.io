...
Web site는 **server**와 **client**가 서로 요청(request)하고 응답(response)하며 동작한다.
...
server가 파일(code)을 보내주고, web browser가 실행한다.  
...
## 📖 2. Web Page의 언어, HTML (HyperText Markup Language)

Browser가 읽는 파일의 형식이 **HTML**이다.  
Web page는 모두 HTML code로 만들어진다. (website에서 HTML code를 직접 확인해보자)

VSCode에서 `index.html` 파일을 만들고 아래 내용을 작성하고 web browser에서 열어 실행해 보자.
...
## 📖 3. Web Page의 Design, CSS(Cascading Style Sheets)
...
Study blog를 인터넷에 게시할 수 있는 website를 만들어 보자. 